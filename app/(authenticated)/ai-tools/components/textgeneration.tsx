...
export default function TextGeneration() {
  ...
  useEffect(() => {
    const allSessions = getToolSessionsByType("image-to-text");
    ...
    if (allSessions.length > 0) {
      ...
      const newSession = createToolSession("image-to-text", "Image to Text");
  ...
  const saveResultToSession = async (
    ...
    if (session) {
      ...
      const updated = getToolSessionsByType("image-to-text");
      setSessions(updated);
    }
  };

  // Restore previous result from session
  const restoreFromSession = (session: ToolSession) => {
    if (session.results && session.results.length > 0) {
      const lastResult = session.results[session.results.length - 1];
      setPreview(lastResult.input as string);
      setResult(lastResult.output as string);
  ...
  const handleDeleteSession = (sessionId: string) => {
    ...
    setSessions(getToolSessionsByType("image-to-text"));
  ...
  const handleExtractText = async () => {
    ...
    try {
      ...
      const data = await imageToText(base64, {
        extract: "all",
        format: "markdown",
      });
      const extractedText = data.text || "No text found in image.";
      setResult(extractedText);

      // Save to session
      await saveResultToSession(extractedText, preview);
  ...
  return (
    <div className="flex gap-6 w-full">
      ...
      <div className="hidden lg:block w-56 shrink-0">
        <div className="bg-gradient-to-b from-blue-50 to-blue-50/50 rounded-xl border border-blue-100 p-4 max-h-[600px] overflow-y-auto">
              ...
              recentResults
                ...
                .map((result) => (
                  <motion.div
                    ...
                    className="p-2 rounded-lg bg-white border border-blue-100 hover:border-blue-300 transition cursor-pointer group relative"
                  >
                    <div
                      onClick={() => {
                        setPreview(String(result.input));
                        setResult(String(result.output));
                      }}
                    >
                      <div className="w-full h-12 mb-1 rounded overflow-hidden bg-gray-100">
                        <img
                          src={String(result.input)}
                          alt="Preview"
          ...
          <div className="w-full max-w-4xl rounded-3xl border bg-white shadow-2xl p-8 space-y-8">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-gray-500 text-lg">
                Easily extract text from any image. Upload, drag & drop, or
          ...
          `}
            ...
            <div className="bg-blue-50 border-l-4 border-blue-400 p-4 rounded-xl flex items-start gap-3">
              ...
              <div className="text-sm text-blue-700 space-y-1">
                <p>
                  <strong>Tip:</strong> Use high-quality images for best
                  results.
                </p>
                <p>
                  <strong>Shortcut:</strong> Press <code>Ctrl+V</code> to paste
                  an image from clipboard.
                </p>
                <p>
                  <strong>Pro Tip:</strong> You can extract multiple lines,
                  tables, or even code snippets.
                </p>
              </div>
            </div>
          </div>
